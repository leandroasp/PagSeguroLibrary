 ...
 */
class PagSeguro_PaymentRequest  {
    ...
    */
	...
	public function setSender($name, $email = null, $areaCode = null, $number = null) {
        $param = $name;
		if (is_array($param)) {
        	$this->sender = new PagSeguro_Sender($param);
        } elseif($param instanceof PagSeguro_Sender) {
        	$this->sender = $param;
        } else {
        	$sender = new PagSeguro_Sender();
        	$sender->setName($param);
        	$sender->setEmail($email);
        	$sender->setPhone(new PagSeguro_Phone($areaCode, $number));
        	$this->sender = $sender;
        }
    }
    ...
    public function setSenderName($senderName) {
    	if ($this->sender == null) {
            $this->sender = new PagSeguro_Sender();
        }
    ...
    public function setSenderPhone($areaCode, $number = null) {
    	...
    	if ($param instanceof PagSeguro_Phone) {
    		$this->sender->setPhone($param);
    	} else {
    		$this->sender->setPhone(new PagSeguro_Phone($param, $number));
    	}
    }
    ...
    public function setItems(Array $items) {
    	if (is_array($items)) {
    		...
    		foreach ($items as $key => $item) {
    			if ($item instanceof PagSeguro_Item) {
    				$i[$key] = $item;
    			} else if (is_array($item)) {
    				$i[$key] = new PagSeguro_Item($item);
    			}
    		}
			$this->items = $i;
    	}
    ...
    public function addItem($id, $description = null, $quantity = null, $amount = null, $weight = null, $shippingCost = null) {
        ...
        if (is_array($param)) {
        	array_push($this->items, new PagSeguro_Item($param));
        } else if ($param instanceof PagSeguro_Item) {
        	array_push($this->items, $param);
        } else {
        	$item = new PagSeguro_Item();
        	$item->setId($param);
    ...
    public function setShipping($address, $type = null) {
    	$param = $address;
    	if ($param instanceof PagSeguro_Shipping) {
    		$this->shipping = $param;
    	} else {
    		$shipping = new PagSeguro_Shipping();
    		if (is_array($param)) {
    			$shipping->setAddress(new PagSeguro_Address($param));
    		} else if ($param instanceof PagSeguro_Address) {
    			$shipping->setAddress($param);
    		}
    		if ($type) {
    			if ($type instanceof PagSeguro_ShippingType) {
    				$shipping->setType($type);
    			} else {
    				$shipping->setType(new PagSeguro_ShippingType($type));
    			}    		
    		}
    ...
    public function setShippingAddress($postalCode = null, $street = null, $number = null, $complement = null, $district = null, $city = null, $state = null, $country = null) {
    	...
    	if ($this->shipping == null) {
			$this->shipping = new PagSeguro_Shipping();
		}
		if (is_array($param)) {
			$this->shipping->setAddress(new PagSeguro_Address($param));
		} elseif ($param instanceof PagSeguro_Address) {
			$this->shipping->setAddress($param);
		} else {
			$address = new PagSeguro_Address();
    ...
    public function setShippingType($type) {
    	...
    	if ($param instanceof PagSeguro_ShippingType) {
    		$this->shipping->setType($param);
    	} else {
    		$this->shipping->setType(new PagSeguro_ShippingType($param));
    	}
    }
    ...
    */
     ...
     */
    public function register(PagSeguro_Credentials $credentials) {
		return PagSeguro_PaymentService::createCheckoutRequest($credentials, $this);
    }
    ...
    */
	public function toString(){
		...
		return "PagSeguro_PaymentRequest(Reference=".$this->reference.",     SenderEmail=".$email.")";
	}
