...
class PagSeguro_Config{
	...
	private function __construct() {
		...
		require_once PagSeguro_Library::getPath().DIRECTORY_SEPARATOR."ConfigParams.php";
	...
	public static function init() {
		if (self::$config == null) {
			self::$config = new PagSeguro_Config();
		}
		return self::$config;
	}
	...
	public static function getAccountCredentials() {
		if (isset(self::$data['credentials']) && isset(self::$data['credentials']['email']) && isset(self::$data['credentials']['token'])) {
			return new PagSeguro_AccountCredentials(self::$data['credentials']['email'], self::$data['credentials']['token']);
		} else {
